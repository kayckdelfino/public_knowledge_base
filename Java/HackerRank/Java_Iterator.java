/*
Challenge statement: https://www.hackerrank.com/challenges/java-iterator/problem
...
public class Java_Iterator {
    static Iterator<Object> func(ArrayList<Object> mylist) {
        ...
        if (index >= 0) {
            return mylist.subList(index + 1, mylist.size()).iterator();
        } else {
    ...
    public static void main(String[] args) {
        ...
        combinedList.add("###");
        for (int i = 0; i < m; i++) {
            combinedList.add(sc.next());
        }

        sc.close();
        Iterator<Object> it = func(combinedList);
        while (it.hasNext()) {
