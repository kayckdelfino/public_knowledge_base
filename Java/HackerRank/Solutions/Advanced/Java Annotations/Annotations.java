/*
Challenge statement: https://www.hackerrank.com/challenges/java-annotations/problem
...
@interface FamilyBudget {
	String userRole() default "GUEST";
	int budgetLimit() default 0; // added
}
...
public class Annotations {
    public static void main(String[] args) {
		...
		while (testCases > 0) {
			...
			int spend = in.nextInt();
			try {
				Class<?> annotatedClass = FamilyMember.class; // <?> added (not part of the solution)
				Method[] methods = annotatedClass.getMethods();
				for (Method method : methods) {
					if (method.isAnnotationPresent(FamilyBudget.class)) {
						FamilyBudget family = method
								.getAnnotation(FamilyBudget.class);
						String userRole = family.userRole();
						int budgetLimit = family.budgetLimit(); // added
						if (userRole.equals(role)) {
							if(budgetLimit >= spend){ //added
                                // next 4 lines changed to prevent newInstance() from being deprecated (not part of the solution)
								Constructor<?> con = annotatedClass.getDeclaredConstructor();
                                con.setAccessible(true);
                                Object familyMember = con.newInstance();
        ...
        in.close(); // added (not part of the solution)
	}
}